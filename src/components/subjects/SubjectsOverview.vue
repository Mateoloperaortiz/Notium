...
function semesterName(semesterId: string): string {
	return userSemesters.value.find((semester) => semester.id === semesterId)?.name ?? 'Semestre no encontrado'
}
			...
			<div v-else class="space-y-3">
				...
				>
					<div class="flex flex-wrap items-start justify-between gap-3">
						<div>
							<p class="text-xs font-semibold tracking-wide text-brand-700 uppercase">{{ subject.code }}</p>
